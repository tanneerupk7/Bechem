...
const SOGTable = ({ accountId, isAdmin, accountName, selectedDistributor }) => {
  ...
  const fetchPendingApprovals = async () => {
    ...
    try {
      const response = await fetch(`${API_URI}/pending_approvals/`, {
        method: "POST",
        headers: {
      ...
      setErrorMessage("An error occurred while fetching pending approvals.");
  ...
  useEffect(() => {
    if (accountId || (selectedDistributor && selectedDistributor.ac_id)) {
    ...
    <div className="h-screen flex flex-col">
      {loading && (
        <div className="fixed top-0 left-0 w-full h-full flex flex-col items-center justify-center bg-black bg-opacity-50 z-50">
          ...
          <p className="mt-4 text-white">Loading pending approvals...</p>
        </div>
      )}
      <Header
        isAdmin={isAdmin}
              ...
              >
                <span className="mr-2">
                  ...
                  >
                    ...
                    />
                    <path d="M10 10V17.5" className="stroke-1.25" />
                    <path
          ...
          <table className="table-auto w-full border-collapse border-b border-gray-300 text-xs border-l border-r">
            <thead>
              <tr className="bg-tableHeaderColor border-b border-gray-300">
                ...
                <th
                  className="p-2 font-normal text-left border-r border-gray-300"
                  style={{ width: "15%" }}
                >
                  SOG Reason
                </th>
            ...
            <tbody>
              {currentData.map((data, index) => (
                ...
                >
                  <td className="p-2 border-l border-gray-300 overflow-hidden whitespace-nowrap text-left">
                    {data.tran_cd}
                  </td>
                  ...
                  <td className="p-2 text-left border-r border-gray-300 overflow-hidden whitespace-nowrap">
                    {data.SOG_REASON}
                  </td>
                </tr>
              ))}
